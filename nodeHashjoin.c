		...
		switch (node->hj_JoinState)
		{
			...
			case HJ_BUILD_HASHTABLE:
                ...
                if (innerPlanState(node)->instrument){
                    ...
                    double hashNode_estimate_ntuples = innerPlanState(node)->plan->plan_rows;
                    double hashNode_actual_ntuples = innerPlanState(node)->instrument->ntuples;
                    double outerNode_estimate_ntuples = outerPlanState(node)->plan->plan_rows;
                    double outerNode_actual_ntuples = outerPlanState(node)->instrument->ntuples;

                    bool needReopt = false;
                    double hashNode_error=0.0;
                    double outerNode_error=0.0;
                    double qerror_threshold = 100.0;

                    //we measure the q-error as threshold to determine re-optimization
                    if (hashNode_actual_ntuples > 1.0){
                        if (hashNode_estimate_ntuples > hashNode_actual_ntuples){
                            hashNode_error = hashNode_estimate_ntuples / hashNode_actual_ntuples;
                        }else{
                            hashNode_error = hashNode_actual_ntuples / hashNode_estimate_ntuples;
                        }
                        if(hashNode_error >= qerror_threshold){
                            elog(INFO,
                            "     ------>------> Fang ExecHashJoinImpl hash node card Q-error larger than 100.0, need to call LPCE-R");
                            needReopt = true;
                        }
                    }
                    if (outerNode_actual_ntuples > 1.0){

                        if (outerNode_estimate_ntuples > outerNode_actual_ntuples){
                            outerNode_error = outerNode_estimate_ntuples / outerNode_actual_ntuples;
                        }else{
                            outerNode_error = outerNode_actual_ntuples / outerNode_estimate_ntuples;
                        }
                        if(outerNode_error >= qerror_threshold)
                             elog(INFO, "     ------>------> Fang ExecHashJoinImpl outer node card Q-error larger than 100.0, need to call LPCE-R");
                    }

                    if(needReopt){
                        if(hashNode_error >= qerror_threshold){
                            //we make hash node materialization view
                            elog(INFO,"     ------>------> Fang ExecHashJoinImpl needs materialization hash node");
                            //extract subplan for CTE
                            //rewrite new query with CTE
                            //submit new query to postgreSQL again
                            //extract execution time from new query


                        }

                        if(outerNode_error >= qerror_threshold){
                            //TO-DO we make outer node materialization view
                            elog(INFO,"     ------>------> Fang ExecHashJoinImpl needs materialization outer node: TO-DO works");
                        }
                    }
                }
			...
			case HJ_SCAN_BUCKET:
				...
				if (joinqual == NULL || ExecQual(joinqual, econtext))
				{
                    ...
                    fangCounter++;
                    if(fangCounter % 100 == 0)
                        elog(INFO, "     ------>------>------> Fang OUTER got a match: %ld", fangCounter);
                ...
                elog(INFO, "     ------>------>------> Fang ExecHashJoinImpl HJ_SCAN_BUCKET");
				break;
			...
			case HJ_NEED_NEW_BATCH:
				...
				node->hj_JoinState = HJ_NEED_NEW_OUTER;
                //elog(INFO, "     ------>------>------> Fang ExecHashJoinImpl HJ_NEED_NEW_BATCH");
				break;
